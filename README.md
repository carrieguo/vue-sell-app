...
> `Css Sticky footers 布局` 效果：在浮层中，将footer固定到底部。文章内容不足满屏时 footer在底部，超过满屏时footer在内容末尾。
...
> 尽量用class去写样式而不是用标签，class查找性能要好于标签, 特别是层级嵌套比较深的情况下。
...
> vue动画 transittion
> 计算属性 `selectFoods` 观测的是data重的goods, goods由food组成。一旦food发生变化，goods相应变化，计算属性 `selectFoods`重新计算。
